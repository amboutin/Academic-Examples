...
int main(int argc, char *argv[])
{
	...
	if (argc < 4) { fprintf(stderr, "USAGE: %s cyphertext keyfile port\n", argv[0]); exit(1); } // Check usage & args

	/*************************************************************************************************/

	// Get cyphertext from file
	...
	// Validate Cyphertext against bad characters
	int i;
	for (i = 0; i < strlen(buffer) - 1; i++) {
		if (buffer[i] < 'A' || buffer[i] > 'Z') {
			if (buffer[i] != ' ')
			{
				fprintf(stderr, "otp_dec error : %s contains bad characters", argv[1]);
	...
	serverHostInfo = gethostbyname("localhost");//(argv[1]); // Convert the machine name into a special form of address
	...
	{
		fprintf(stderr, "CLIENT: ERROR connecting to port %d", portNumber);
		exit(2);
	}
	...
	if (verify[0] != 'C') { 
		error("otp_dec: Incompatible Connection Attempt with opt_enc_d");
		exit(1);
	}

	/*************************************************************************************************/
										 
	// Send Plaintext to otp_dec_d
	...
	while (alpha != '*') {
		...
		charsRead = recv(socketFD, buffer2, sizeof(buffer2) - 1, 0); // Read the servers's message from the socket
		alpha = buffer2[strlen(buffer2) - 1];
	}
	buffer2[strcspn(buffer2, "*")] = '\0'; // Remove the trailing identifier and replace with terminator
	fprintf(stdout, "%s\n", buffer2);
